...
def read_timesheet_template(file_path):
    timesheet_df = pd.read_excel(file_path, sheet_name='Your_Timesheet_Sheet_Name')  # Replace with your sheet name
    return timesheet_df

def prefill_of_numbers(timesheet_df, project_info):
    for project_code, details in project_info.items():
        row_index = timesheet_df[timesheet_df['Project Code Column'] == project_code].index
        if row_index.empty:
            continue
        of_code = details['OF Codes']['INTERNE'] or details['OF Codes']['EXTERNE']
        timesheet_df.at[row_index, 'OF Number Column'] = of_code  # Replace 'OF Number Column'
    return timesheet_df

def main():
    planning_file_path = 'Planning SAV.xlsx'  # Replace with your file path
    timesheet_file_path = 'Feuille_pointage_.xlsx'  # Replace with your file path
    ...
    timesheet_df = read_timesheet_template(timesheet_file_path)
    updated_timesheet_df = prefill_of_numbers(timesheet_df, project_info)

    updated_timesheet_df.to_excel('updated_timesheet.xlsx', index=False)
    print("Updated timesheet saved to 'updated_timesheet.xlsx'.")
